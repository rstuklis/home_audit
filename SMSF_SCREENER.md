...
The screener itself uses only the Python standard library, like
`home_net_audit.py`, so it runs on a stock macOS Python. Downloading price
history needs the third-party `yfinance` package.

```sh
python3 -m pip install yfinance                       # once, for --fetch only
python3 smsf_screener.py --fetch --data-dir data      # download 3 years of history, then screen
python3 smsf_screener.py --data-dir data              # screen again from the cached files
python3 smsf_screener.py --data-dir data --explain VAS
python3 smsf_screener.py --data-dir data --illustrate balanced --n-parcels 6 --parcel 15000 --balance 500000
...
Add `--brokerage 9.95` (or whatever your broker charges) so the unit counts and
leftover cash are right. Add `--as-of YYYY-MM-DD` to freeze the calculation
date so two runs on different days agree. `--json out.json` and
...
`--fetch` writes these from Yahoo Finance (`<TICKER>.AX`). Yahoo's coverage of
...
from an issuer's distribution history if a series looks wrong.
